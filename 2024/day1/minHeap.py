...
class minHeap(list):
    ...
    def _heapify_up(self, index):
        ...
        parent = (index - 1) // 2
        if parent >= 0 and self[index] < self[parent]:
    ...
    def _heapify_down(self, index):
        ...
        if left < len(self) and self[left] < self[smallest]:
            smallest = left
        if right < len(self) and self[right] < self[smallest]:
            smallest = right
        if smallest != index:
...
def main():
    ...
    for line in sys.stdin:
        ...
        heap_1.push(int(first.strip()))
        heap_2.push(int(second.strip()))
    
    distance = 0
    length = len(heap_1)
    for _ in range(length):
        distance += abs(heap_1.pop() - heap_2.pop())
